...
- entry detail/raw blob page cache 的 cache entry 类型、加载 helper 和释放 helper 已迁入 `ezdb_entries.c/.h`；base raw blob range copy helper 和 delta blob range copy helper 已迁出，`load_entry_detail` 仍待继续迁出。
