...
- entry path/raw path copy helpers（已迁出 base raw blob range copy helper 和 delta blob range copy helper）
...
3. 迁出 `entry_path_copy_by_id`、`load_entry_detail`。（已完成 `load_entry_detail` 读取逻辑迁出）
4. 迁出 compact entry source。
5. 与 v13 section descriptor 对接。
...
- entry detail/raw blob page cache 的 cache entry 类型、加载 helper 和释放 helper 已迁入 `ezdb_entries.c/.h`；entry detail reader helper、base raw blob range copy helper 和 delta blob range copy helper 已迁出，`entry_path_copy_by_id` 仍待继续迁出。
